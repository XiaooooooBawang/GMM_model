...
from pydpm._model._basic_model import Basic_Model
...
class GMM(Basic_Model):
    ...
    def train(self, iter_num: int, data: np.ndarray, is_train: bool = True):
        """
        Inputs:
            iter_num   : [int] scalar, the iterations of GMM
            data       : [np.ndarray] N*D matrix
        ...
        cluster = self.global_params.Weight.argmax(axis=1).tolist()
    ...
    def e_step(self, data):
        for i in range(self._model_setting.K):
            ...
            ).pdf(data) * self.global_params.Phi[0][i]
